...
int
ConnectMpi::MPI_Recv_int( int* int_val, int n, int sender_id )
{
  MPI_Status Stat;
  int tag = 1;
...
int
ConnectMpi::MPI_Recv_float( float* float_val, int n, int sender_id )
{
  MPI_Status Stat;
  int tag = 1;
...
int
ConnectMpi::MPI_Recv_uchar( unsigned char* uchar_val, int n, int sender_id )
{
  MPI_Status Stat;
  int tag = 1;

  MPI_Recv( uchar_val, n, MPI_UNSIGNED_CHAR, sender_id, tag, MPI_COMM_WORLD, &Stat );

  return 0;
}

int
ConnectMpi::MPI_Send_int( int* int_val, int n, int target_id )
{
  int tag = 1;
...
int
ConnectMpi::MPI_Send_float( float* float_val, int n, int target_id )
{
  int tag = 1;
...
int
ConnectMpi::MPI_Send_uchar( unsigned char* uchar_val, int n, int target_id )
{
  int tag = 1;
...
int
ConnectMpi::MpiInit( int argc, char* argv[] )
{
  int initialized;
...
bool
ConnectMpi::ProcMaster()
{
  if ( mpi_id_ == mpi_master_ )
  {
    return true;
  }
  else
