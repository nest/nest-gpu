...
int
BaseNeuron::Init( int i_node_0, int n_node, int n_port, int i_group, unsigned long long* seed )
{
...
BaseNeuron::ActivateRecSpikeTimes( int max_n_rec_spike_times )
{
  ...
  { // add it if not already present
    ...
    gpuErrchk( cudaMalloc( &n_rec_spike_times_cumul_, ( n_node_ + 1 ) * sizeof( int ) ) );
    ...
    gpuErrchk( cudaMalloc( &rec_spike_times_, n_node_ * max_n_rec_spike_times * sizeof( int ) ) );
    gpuErrchk( cudaMalloc( &rec_spike_times_pack_, n_node_ * max_n_rec_spike_times * sizeof( int ) ) );
