...
__global__ void
NodeGroupMapInit( signed char* node_group_map )
{
...
{
  gpuErrchk( cudaMalloc( &d_node_group_map_, node_group_map_.size() * sizeof( signed char ) ) );
  ...
  gpuErrchk( cudaMemcpyAsync( d_node_group_map_,
    node_group_map_.data(),
...
NESTGPU::InitGetSpikeArray( int n_node, int n_port )
{
  ...
  if ( n_node * n_port > 0 )
  {
    gpuErrchk( cudaMalloc( &d_get_spike_array, n_node * n_port * sizeof( double ) ) );
  }
