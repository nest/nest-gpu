...
int
NESTGPU::Connect( int i_source_node,
  int i_target_node,
  unsigned char port,
  ...
  float delay )
{
  ...
  net_connection_->Connect( i_source_node, i_target_node, port, syn_group, weight, delay );
...
template <>
int
NESTGPU::_SingleConnect< int, int >( int i_source0,
  int i_source,
  int i_target0,
  int i_target,
  float weight,
  ...
  SynSpec& syn_spec )
{
  return net_connection_->Connect(
    i_source0 + i_source, i_target0 + i_target, syn_spec.port_, syn_spec.syn_group_, weight, delay );
}
...
template <>
int
NESTGPU::_RemoteSingleConnect< int >( int i_source,
  int i_target0,
  int i_target,
  float weight,
  float delay,
  ...
  SynSpec& syn_spec )
{
  ...
  RemoteConnection rc = { i_source, i_target0 + i_target, syn_spec.port_, syn_spec.syn_group_, weight, delay };
...
int
NESTGPU::Connect( int i_source, int n_source, int i_target, int n_target, ConnSpec& conn_spec, SynSpec& syn_spec )
{
  return _Connect< int, int >( i_source, n_source, i_target, n_target, conn_spec, syn_spec );
}

int
NESTGPU::Connect( int i_source, int n_source, int* target, int n_target, ConnSpec& conn_spec, SynSpec& syn_spec )
{
  return _Connect< int, int* >( i_source, n_source, target, n_target, conn_spec, syn_spec );
}
int
NESTGPU::Connect( int* source, int n_source, int i_target, int n_target, ConnSpec& conn_spec, SynSpec& syn_spec )
{
  return _Connect< int*, int >( source, n_source, i_target, n_target, conn_spec, syn_spec );
}
int
NESTGPU::Connect( int* source, int n_source, int* target, int n_target, ConnSpec& conn_spec, SynSpec& syn_spec )
{
  return _Connect< int*, int* >( source, n_source, target, n_target, conn_spec, syn_spec );
}

int
NESTGPU::Connect( NodeSeq source, NodeSeq target, ConnSpec& conn_spec, SynSpec& syn_spec )
{
  return _Connect< int, int >( source.i0, source.n, target.i0, target.n, conn_spec, syn_spec );
}

int
NESTGPU::Connect( NodeSeq source, std::vector< int > target, ConnSpec& conn_spec, SynSpec& syn_spec )
{
  return _Connect< int, int* >( source.i0, source.n, target.data(), target.size(), conn_spec, syn_spec );
}

int
NESTGPU::Connect( std::vector< int > source, NodeSeq target, ConnSpec& conn_spec, SynSpec& syn_spec )
{
  return _Connect< int*, int >( source.data(), source.size(), target.i0, target.n, conn_spec, syn_spec );
}

int
NESTGPU::Connect( std::vector< int > source, std::vector< int > target, ConnSpec& conn_spec, SynSpec& syn_spec )
{
  return _Connect< int*, int* >( source.data(), source.size(), target.data(), target.size(), conn_spec, syn_spec );
}
  ...
  SynSpec& syn_spec )
{
  ...
  return _RemoteConnect< int, int >( rsource, n_source, rtarget, n_target, conn_spec, syn_spec );
#else
  ...
  SynSpec& syn_spec )
{
  ...
  return _RemoteConnect< int, int >( rsource, source.n, rtarget, target.n, conn_spec, syn_spec );
#else
  throw ngpu_exception( "MPI is not available in your build" );
#endif
}
  ...
  SynSpec& syn_spec )
{
  ...
  return _RemoteConnect< int, int* >( rsource, source.n, rtarget, target.size(), conn_spec, syn_spec );
#else
  throw ngpu_exception( "MPI is not available in your build" );
#endif
}
  ...
  SynSpec& syn_spec )
{
  ...
  RemoteNode< int > rtarget( i_target_host, target.i0 );
  return _RemoteConnect< int*, int >( rsource, source.size(), rtarget, target.n, conn_spec, syn_spec );
#else
  throw ngpu_exception( "MPI is not available in your build" );
#endif
}
  ...
  SynSpec& syn_spec )
{
  ...
  RemoteNode< int* > rsource( i_source_host, source.data() );
  RemoteNode< int* > rtarget( i_target_host, target.data() );
  return _RemoteConnect< int*, int* >( rsource, source.size(), rtarget, target.size(), conn_spec, syn_spec );
#else
  throw ngpu_exception( "MPI is not available in your build" );
#endif
}
