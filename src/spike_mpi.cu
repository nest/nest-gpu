...
__global__ void
PushSpikeFromRemote( int n_spikes, int* spike_buffer_id, float* spike_height, int offset )
{
...
__device__ void
PushExternalSpike( int i_source, float height )
{
...
int
ConnectMpi::ExternalSpikeInit( int n_node, int n_hosts, int max_spike_per_host )
{
  ...
  gpuErrchk( cudaMalloc( &d_ExternalSpikeSourceNode, max_spike_per_host * sizeof( int ) ) );
  ...
  gpuErrchk( cudaMalloc( &d_ExternalTargetSpikeNodeId, n_hosts * max_spike_per_host * sizeof( int ) ) );
  gpuErrchk( cudaMalloc( &d_ExternalTargetSpikeHeight, n_hosts * max_spike_per_host * sizeof( float ) ) );
  // gpuErrchk(cudaMalloc(&d_ExternalSourceSpikeNum, n_hosts*sizeof(int)));
  gpuErrchk( cudaMalloc( &d_ExternalSourceSpikeNodeId, n_hosts * max_spike_per_host * sizeof( int ) ) );
  gpuErrchk( cudaMalloc( &d_ExternalSourceSpikeHeight, n_hosts * max_spike_per_host * sizeof( float ) ) );

  gpuErrchk( cudaMalloc( &d_ExternalTargetSpikeNodeIdJoin, n_hosts * max_spike_per_host * sizeof( int ) ) );
  ...
  for ( int i_source = 0; i_source < n_node; i_source++ )
  {
    ...
    if ( Nth > 0 )
    {
      gpuErrchk( cudaMalloc( &h_ExternalNodeTargetHostId[ i_source ], Nth * sizeof( int ) ) );
      ...
      cudaMemcpyAsync(
        h_ExternalNodeTargetHostId[ i_source ], target_host_arr, Nth * sizeof( int ), cudaMemcpyHostToDevice );
      cudaMemcpy( h_ExternalNodeId[ i_source ], node_id_arr, Nth * sizeof( int ), cudaMemcpyHostToDevice );
      delete[] target_host_arr;
      delete[] node_id_arr;
    }
  }
  cudaMemcpyAsync(
    d_NExternalNodeTargetHost, h_NExternalNodeTargetHost, n_node * sizeof( int ), cudaMemcpyHostToDevice );
  cudaMemcpyAsync(
    d_ExternalNodeTargetHostId, h_ExternalNodeTargetHostId, n_node * sizeof( int* ), cudaMemcpyHostToDevice );
  cudaMemcpyAsync( d_ExternalNodeId, h_ExternalNodeId, n_node * sizeof( int* ), cudaMemcpyHostToDevice );

  DeviceExternalSpikeInit<<< 1, 1 >>>( n_hosts,
    max_spike_per_host,
    d_ExternalSpikeNum,
...
__global__ void
DeviceExternalSpikeInit( int n_hosts,
...
int
ConnectMpi::SendSpikeToRemote( int n_hosts, int max_spike_per_host )
{
  MPI_Request request;
  int tag = 1;
  ...
  gpuErrchk(
    cudaMemcpy( h_ExternalTargetSpikeNum, d_ExternalTargetSpikeNum, n_hosts * sizeof( int ), cudaMemcpyDeviceToHost ) );
...
int
ConnectMpi::RecvSpikeFromRemote( int n_hosts, int max_spike_per_host )
...
int
ConnectMpi::CopySpikeFromRemote( int n_hosts, int max_spike_per_host, int i_remote_node_0 )
{
  double time_mark = getRealTime();
  int n_spike_tot = 0;
  // loop on MPI proc
  for ( int i_host = 0; i_host < n_hosts; i_host++ )
  {
    ...
    {
      // pack spikes received from remote MPI processes
      h_ExternalSourceSpikeNodeId[ n_spike_tot ] = h_ExternalSourceSpikeNodeId[ i_host * max_spike_per_host + i_spike ];
      n_spike_tot++;
    }
  }
  ...
  if ( n_spike_tot > 0 )
  {
    ...
    gpuErrchk( cudaMemcpyAsync(
      d_ExternalSourceSpikeNodeId, h_ExternalSourceSpikeNodeId, n_spike_tot * sizeof( int ), cudaMemcpyHostToDevice ) );
...
__global__ void
JoinSpikeKernel( int n_hosts,
  int* ExternalTargetSpikeCumul,
  ...
  int max_spike_per_host )
{
  ...
  if ( array_idx < n_spike_tot )
  {
    ...
    // packed spike array
    ExternalTargetSpikeNodeIdJoin[ array_idx ] = ExternalTargetSpikeNodeId[ i_host * max_spike_per_host + i_spike ];
  }
}
...
int
ConnectMpi::JoinSpikes( int n_hosts, int max_spike_per_host )
{
  double time_mark = getRealTime();
  ...
  prefix_scan( d_ExternalTargetSpikeCumul, d_ExternalTargetSpikeNum, n_hosts + 1, true );
