...
params = {
    ...
    'seed': 123456,          # seed for random number generation
...
def build_network():
    ...
    nu_thresh = model_params['Theta_rel'] / ( CE_distrib * model_params['tau_m'] / model_params['C_m'] * JE_pA * np.exp(1.) * tau_syn)
    nu_ext = nu_thresh * brunel_params['eta']

    rate = nu_ext * CE_distrib * 1000.
    ...
    ngpu.Connect(E_stim, neuron[mpi_id], {'rule': 'all_to_all'}, {'weight': JE_pA, 'delay': brunel_params['delay']})
    ...
    #print("Connections to neuron[mpi_id] [local conn]: ", len(ngpu.GetConnections(E_pops[mpi_id][0])))
    dum = True #remote connections
    ...
    #print("Connections to neuron[mpi_id] [local&remote conn]: ", len(ngpu.GetConnections(E_pops[mpi_id][0])))
...
def run_simulation():
    ...
    if params['record_spikes']:
        ...
        print(len(spike_times[0]),len(spike_times[25]),len(spike_times[-1]))
