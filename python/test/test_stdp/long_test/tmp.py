...
N=2 #400 # number of presynaptic and postsynaptic neurons
...
for j in range(N):
    for i in range(N):
        ...
        print(i, neuron_pre[i], neuron_post[j])
        ...
        if (Dt > 1.0e-6) | (Dt<-1.0e-6):
            ...
            if abs(dw_rel)>0.01:
                #print(neuron_pre[i], neuron_post[j], delay_pre, delay_post)
                #if neuron_pre[i]==200 and neuron_post[j]==500:
                print(i, j, neuron_pre[i], neuron_post[j])
                print("Initial weight: ", weight_stdp)
                print("Simulated weight: ", w[0])
                print("Expected theoretical weight: ", w1)
                print("dw/w: ", dw_rel) 
...
status_list = ngpu.GetStatus(conn_id)
for conn_stat in status_list:
    i_source = conn_stat['source']
    i_target = conn_stat['target']
    weight = conn_stat['weight']
    delay = conn_stat['delay']
    den_delay = ngpu.GetStatus([i_target], 'den_delay')[0]
    print (conn_stat)
    print (i_source, i_target, delay, den_delay, weight)
    conn_sg_src = ngpu.GetConnections(sg, [i_source])
    conn_sg_tgt = ngpu.GetConnections(sg, [i_target])
    #print(ngpu.GetStatus(conn_sg_src)[0])
    #print(ngpu.GetStatus(conn_sg_tgt)[0])
    src_delay = ngpu.GetStatus(conn_sg_src, 'delay')[0]
    tgt_delay = ngpu.GetStatus(conn_sg_tgt, 'delay')[0]
    print (src_delay, tgt_delay)
    t_diff = tgt_delay + den_delay - (src_delay + delay)
    print (t_diff)
    print
